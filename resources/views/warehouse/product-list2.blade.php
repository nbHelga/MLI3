...
<div class="py-6" 
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach($barang as $item)
                            <tr>
                                ...
                                <td class="px-6 py-4">{{ $item->kode }} (IND)</td>
                                <td class="px-6 py-4">{{ $item->nama }} (IND)</td>
                                <td class="px-6 py-4">{{ $item->kualitas }}</td>
                                <td class="px-6 py-4">{{ $item->size }}</td>
                                {{-- <td class="px-6 py-4">{{ $item->jumlah }}</td> --}}
                                <td class="w-12 px-3 py-4">
                                    <x-elements.advanced-barang :kode="$item->kode":isBarangPallet="false" />
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
    ...
    <!-- Pop-up Message -->
    @if(session('error'))
        <x-pop-up-message type="error" message="Data NOT updated successfully" />
    @endif
</div>
@endsection