@props(['id', 'barangPallet'])

<div x-data="{ 
    open: false,
    showDeleteDialog: false,
    async deleteBarangGudang(id) {
        console.log('Deleting Barang Gudang:', id);
        try {
            const response = await axios.delete(`/warehouse/barangpallet/${id}`);
            console.log('Delete Response:', response.data);
            if (response.data.success) {
                const tempat = '{{ strtolower($barangPallet->tempat->nama) }}';
                window.location.href = `/warehouse/barangpallet-list/${tempat}?delete_success=true`;
            }
        } catch (error) {
