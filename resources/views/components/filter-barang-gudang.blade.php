...
<form x-data="{
...
}" class="relative" method="GET">
    <!-- Preserve existing URL parameters -->
    @foreach(request()->except(['pallet', 'kategori', 'kualitas', 'size', 'page']) as $key => $value)
        @if(is_array($value))
    ...
    <div x-show="showFilter"
         ...
         class="fixed top-1/2 right-4 transform -translate-y-1/2 z-50 w-max bg-white p-4 rounded-lg shadow-lg border border-gray-200"
        ...
        <div class="flex space-x-6">
            <!-- Pallet -->
            <div class="w-64">
                <div class="font-medium mb-2">Pallet</div>
                <div class="border-b border-gray-200 mb-4"></div>
                <div class="space-y-2 max-h-[300px] overflow-y-auto">
                    <label class="flex items-center">
                        <input type="checkbox" 
                               @click="categories['Pallet'].items.forEach(item => item.checked = !categories['Pallet'].selectAll)"
                               x-model="categories['Pallet'].selectAll">
                        <span class="ml-2">Select All</span>
                    </label>
                    <div class="grid grid-cols-2 gap-2">
                        <template x-for="item in categories['Pallet'].items" :key="item.name">
                            <label class="flex items-center">
                                <input type="checkbox" 
